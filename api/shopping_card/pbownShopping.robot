...
***Test Cases***
User buy some toys for his/her daughter and choose the delivery by Kerry and choose the payment by Visa and successfull pay
    Search
    Product Details
    Submit Order
    Confirm payment
...
Search
    ...
    ${productId}=    Set Variable    ${resp.json()['products'][1]['id']}
    Set Test Variable    ${productId}

Product Details
    ${resp}=    Get Request    alias=shopping    uri=/api/v1/product/${productId}    headers=&{headers}
    Request Should Be Successful    ${resp}
